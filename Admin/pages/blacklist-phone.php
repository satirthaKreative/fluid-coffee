      ...
      <div class="row">
        <div class="col-12">
          <div class="card">
            <div class="row">
              <div class="col-4">
                <button type="button" class="btn btn-success mr20" onclick="add_campaign()">Add Blacklist Phone Numbers</button>
              </div>
            </div>
            <div class="card-header">
              <h3 class="card-title">Blacklist Phone</h3>
            ...
            <div class="card-body table-responsive p-0">
              <table class="table table-hover table-striped">
                ...
                while($fetchCate = $selectPhone->fetch_array()){ 
                ?>
                  <tr>
                    <td><?= $i; ?></td>
                    <td><?= $fetchCate['cate_name'] ?></td>
                    <td><?= date('d M,Y',strtotime($fetchCate['added_on'])); ?></td>
                    <td><button type="button" onclick="change_status(<?= $fetchCate['id']; ?>,1)" class="btn btn-success btn-sm activity-btn activity-active <?php if($fetchCate['status'] == 1){ ?> activity-btn-active <?php } ?> mr9" data-toggle="tooltip" title="Active"></button>  <button onclick="change_status(<?= $fetchCate['id']; ?>,0)" type="button" class="btn btn-danger btn-sm activity-btn activity-active <?php if($fetchCate['status'] == 0){ ?> activity-btn-active <?php } ?>" data-toggle="tooltip" title="Inactive"></button> </td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Add Banned Ip's</h5>
...
<script>
  function add_campaign()
  {
    ...
    $(".modal-title").html("Add Blacklist Phone");
    $(".modal-body").html('<form><div class="form-group"><label for="exampleInputEmail1">Blacklist Phone</label><input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter Blacklist Phone"><small id="emailHelp" class="form-text color-red">Blacklist Phone Add.</small></div></form>');
  ...
  function save_data()
  {
    ...
    var action_type = 'add';
    $.ajax({
      url: 'ajax-page/blacklistphone.php',
      type: 'post',
  ...
  function edit_catgory(data)
  {
    ...
    $(".modal-title").html("Edit Blacklist Phones");
  ...
  {
    $.ajax({
      url: 'ajax-page/view_blacklistphone.php',
      type: 'post',
      data: {data_val: data},
      dataType: 'json',
      success:  function(event)
      {
        ...
        $(".modal-body").html('<form><div class="form-group"><label for="exampleInputEmail1">Edit Blacklist Phone</label><input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="'+event.cate_name+'" placeholder="Enter Blacklist Phone"><small id="emailHelp" class="form-text color-red">Campaign Blacklist Phone Add.</small></div></form>');
      }
    })
  }
  // update category
  ...
  function del_catgory(data)
  {
     ...
     $(".modal-title").html("Delete Email");
     $(".modal-body").html("<p>Are You really want to delete this email ?</p>");
     $("#add_modal").modal('show');
  ...
  function change_status(data, status_id)
  {
    ...
    $.ajax({
      ...
      success:  function(event)
      {
        ...
        if(event.no_error == true)
        {
          $(".modal-body").html("<p class='color-green'><center ><i class='fa fa-check  suc-modal-pop'></i><br/>Status Successfully Changed</center></p>");
          $(".modal-header").hide();
          $(".modal-footer").hide();
          $("#add_modal").modal('show');
          setTimeout(function(){ window.location.href='<?= $lastPart; ?>'; }, 3000);
